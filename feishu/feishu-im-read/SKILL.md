...
- 所有消息读取工具均以 `user_access_token`（用户身份）调用，只能读取用户有权限的会话
...
- 消息内容中出现资源标记时，用 `feishu_im_user_fetch_resource` 下载，需要 `message_id` + `file_key` + `type`
...
消息内容（`content` 字段）中可能出现以下资源标记，需要用 `feishu_im_user_fetch_resource` 下载：
